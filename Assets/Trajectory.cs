...
public class Trajectory : MonoBehaviour
{
    ...
    IEnumerator beginCurve()
    {
        ...
        float missileRadius = transform.position.y;
        Debug.Log("Missile Radius: " + missileRadius);

        Vector3 newPos = transform.position;

        while((distanceTraveled <= totalDistance))
        {
            Vector3 deltaPath = pathPosition * (((totalDistance/1) * 0.05f)*missileSpeed * Time.deltaTime);
            newPos += deltaPath;
    ...
    {
        Debug.Log("CALLED ON COLLISION TRAJ");
        if(collision.gameObject.tag == "enemy" || collision.gameObject.name == "Ground")
        {
            BeginExplosion();
        }
    ...
    private void BeginExplosion()
    {
        ...
        foreach(var Collider in colliders)
        {
            ...
            if (!(temp.tag == "ally"))
            {
                ...
                } else if (temp.TryGetComponent<BuildingHealth>(out BuildingHealth healthB))
                {
                    Debug.Log("Applying Damage To Building");
                    healthB.takeDamage(20f);
                }
            }
        }
